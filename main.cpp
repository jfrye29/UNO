...
int main(){
    ...
    // Prompt the user to indicate how many players they want
    int NUM_PLAYERS = 0;
    
    while (NUM_PLAYERS < 2) {
        std::cout << "How many players? (>=2)" << std::endl;
        std::cin >> NUM_PLAYERS;
    }
    ...
    } while (discard.at(discard.size() - 1)->getNumber() > 9);
    
    int current = gameState.currentPlayerIndex;
    do {
...
void buildDeck(vector<Card*> &deck){
    ...
    for(int c = RED; c < NUM_COLORS; c++){
        for(int n = 0; n < 10; n++){
            ...
            deck.push_back(temp);
            if (n != 0) deck.push_back(temp);
        }
        
        // Add skip cards (x2)
        Card *skip = new SkipCard((Color)c);
        deck.push_back(skip);
        deck.push_back(skip);
        
        // Add reverse cards (x2)
        Card *reverse = new ReverseCard((Color)c);
        deck.push_back(reverse);
        deck.push_back(reverse);
        
        // Add draw-2 cards (x2)
        Card *draw2 = new Draw2Card((Color)c);
        deck.push_back(draw2);
        deck.push_back(draw2);
    }
    ...
    if(hand.size() > 0){
        for(int i = 0; i <= 8; i++){
            for(int j = 0; j < hand.size(); j++){
                //if (i == 8) cout << "Card " << j << "         ";
                /*else*/ cout << hand.at(j)->render(i, j) << " ";
            }
            cout << endl;
        }
    } else {
...
void takeTurn(vector<Card*> &deck, vector<Card*> &hand, vector<Card*> &discard, GameState &gameState){
    ...
    // If it is empty, we will reset the discard pile (pretty much copy it over to the main deck)
    if (deck.size() == 0) {
        Card *top = discard.at(discard.size() - 1);
